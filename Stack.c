...
{
  st->size = 0;
  st->list =  create_ppl(NULL,NULL,NULL,0,MAX,-1);
}
...
bool empty(stack *st, int vr)
{
  if(st!=NULL){
   if(st->list->heads[vr] == NULL)
      return true;
    return false;
    }
    return true;
...
int pop(stack* st)
{
   if(st!=NULL){
    if(st->size == 0)
     ...
     delete(st->list,st->list->heads[st->list->vr]->value,false);
     st->size = st->size-1;
     return 0; }
     return 1;
}
int top(stack* st, int vr)
{
 if(st!=NULL){
  node* tmp = front(st->list,vr);
  if(tmp != NULL)
    return tmp->value;
    }
  printf("Stack Empty/Unitialized At version %d\n",vr);
  return INT_MIN;
}
void print_stack(stack* st, int vr)
{
   print_list(st->list,vr);
