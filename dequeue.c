...
void init_dequeue(dequeue* q)
{
  q->size = 0;
  q->list =  create_ppl(NULL,NULL,NULL,0,MAX,-1);
}
int push_front(dequeue* q,int val)
{
  if(q!=NULL){
   if(!insert(q->list,val))
     q->size = q->size +1;
   return 0;}
  return 1;
}
...
int push_back(dequeue* q,int val)
{
  if(q!=NULL){
   if(!insert_end(q->list,val))
      q->size = q->size +1;
   return 0;}
  return 1;
}
...
int pop_front(dequeue* q)
{
    ...
    if(q->size == 0)
     ...
     if(!delete(q->list,q->list->heads[q->list->vr]->value,false))
       q->size = q->size-1;
     return 0; }
    return 1;
}
...
int pop_back(dequeue* q)
{
    ...
    if(q->size == 0)
     ...
     if(!delete(q->list,q->list->tail->value,true))
        q->size = q->size-1;
     return 0; }
    return 1;
}
...
int front_dq(dequeue* q, int vr)
{
  ...
  printf("dequeue Empty/UnInitialized At version %d\n",vr);
  return INT_MIN;
}

int back_dq(dequeue* q, int vr)
{
  if(q!=NULL){
  node* tmp = back(q->list,vr);
  if(tmp != NULL)
    return tmp->value;
    }
  printf("dequeue Empty/UnInitialized At version %d\n",vr);
...
void print_dequeue(dequeue* q, int vr)
{
